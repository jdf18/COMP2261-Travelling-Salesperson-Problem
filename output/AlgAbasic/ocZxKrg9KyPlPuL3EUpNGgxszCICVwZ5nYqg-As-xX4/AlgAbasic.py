...
class Population:
    ...
    def __getitem__(self, *args, **kwargs):
        return self.population.__getitem__(*args, **kwargs)
...
PROBABILITY_DIST = [sum(map(linear_selection, range(1, i))) for i in range(1, N+1)]
...
select_parents = select_parents_dist
...
def partially_mapped_crossover(parents):
    ...
    mapping_1 = [i for i in range(num_cities)]
    mapping_2 = [i for i in range(num_cities)]
    for i in range(index_low, index_high):
        swap_1, swap_2 = p_1.tour[i], p_2.tour[i]
        mapping_1[swap_2] = swap_1
        mapping_2[swap_1] = swap_2
    item_map_1 = lambda c:mapping_1[c]
    item_map_2 = lambda c:mapping_2[c]
    print(mapping_1, mapping_2)

    z_1 = list(map(item_map_1, p_1.tour))
    z_1[index_low:index_high] = p_2.tour[index_low:index_high]

    z_2 = list(map(item_map_2, p_2.tour))
    z_2[index_low:index_high] = p_1.tour[index_low:index_high]
...
print(population)
print(list(select_parents(population, 10)))
