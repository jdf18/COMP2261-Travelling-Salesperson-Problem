...
def inv(n):
    if n == 0: return 0
    return 1/n
heuristic_matrix = [[inv(dist_matrix[j][i]) for i in range(num_cities)] for j in range(num_cities)]

class Ant:
    start_node: int
    path: list[int]
    univisted_cities: list[int]
    length: int
    ...
    def choose_next_node(self):
        ...
        def get_probability(node):
            ...
            h = heuristic_matrix[current_node][node] ** beta
            return p * h
        
        probabilities = tuple(accumulate(map(get_probability, self.univisted_cities)))
        choice = random.random() * probabilities[-1]

        return self.univisted_cities[bisect_left(probabilities, choice)]

    def run(self):
        self.univisted_cities = list(range(num_cities))
...
for i in range(max_it):
    ...
    ranking = tuple(sorted(list(map(lambda x:x.length, ants))))
